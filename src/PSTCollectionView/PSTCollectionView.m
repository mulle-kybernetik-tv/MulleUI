...
static inline NSInteger  MAX( NSInteger a, NSInteger b)
{
   return( a < b ? b : a);
}
...
@interface PSTCollectionViewUpdateItem (Internal)
...
static void PSTCollectionViewCommonSetup(PSTCollectionView *_self) {
    ...
    _self->_visibleViewsDict = [NSMutableDictionary new];
    ...
    _self->_extVars = [PSTCollectionViewExt new];
}
...
- (void)layoutSubviews {
    ...
    if (_backgroundView) {
        [_backgroundView setFrame:(CGRect){ .origin = [self contentOffset], 
...
- (void)registerClass:(Class)cellClass forCellWithReuseIdentifier:(NSString * )identifier {
    ...
    [_cellClassDict setObject:cellClass
                       forKey:identifier];
}
...
- (void)registerClass:(Class)viewClass forSupplementaryViewOfKind:(NSString * )elementKind withReuseIdentifier:(NSString * )identifier {
    ...
    [_supplementaryViewClassDict setObject:viewClass
                                    forKey:kindAndIdentifier];}
...
- (id)dequeueReusableCellWithReuseIdentifier:(NSString * )identifier forIndexPath:(NSIndexPath *)indexPath {
    ...
    NSMutableArray * reusableCells = [_cellReuseQueues objectForKey:identifier];
...
//            NSDictionary * externalObjects = [[[self extVars] nibCellsExternalObjects] objectForKey:identifier];
//            if (externalObjects) {
...
- (id)dequeueReusableSupplementaryViewOfKind:(NSString * )elementKind withReuseIdentifier:(NSString * )identifier forIndexPath:(NSIndexPath *)indexPath {
    NSString * kindAndIdentifier = [NSString stringWithFormat:@"%@/%@", elementKind, identifier];
    NSMutableArray * reusableViews = [_supplementaryViewReuseQueues objectForKey:kindAndIdentifier];
...
//            }
            ...
            Class viewClass = [_supplementaryViewClassDict objectForKey:kindAndIdentifier];
...
- (id)dequeueReusableOrCreateDecorationViewOfKind:(NSString * )elementKind forIndexPath:(NSIndexPath *)indexPath {
    NSMutableArray * reusableViews = [_decorationViewReuseQueues objectForKey:elementKind];
    PSTCollectionReusableView *view = [reusableViews lastObject];
...
//            }
         abort();
            ...
            Class viewClass = [decorationViewClassDict objectForKey:elementKind];
            Class reusableViewClass = NSClassFromString(@"UICollectionReusableView");
            if (reusableViewClass && [viewClass isEqual:reusableViewClass]) {
                viewClass = [PSTCollectionReusableView class];
            }
...
- (NSArray *) allCells 
{
   ...
   cls   = [PSTCollectionViewCell class];
   array = [NSMutableArray array];
   for( obj in [_visibleViewsDict allValues])
   {
...
- (NSArray * ) visibleCells 
{
   ...
   for( obj in [_visibleViewsDict allValues])
   {
      if( [obj isKindOfClass:cls])
      {
         if( CGRectIntersectsRect( bounds, [obj frame]))
            [array addObject:obj];
      }
   }
   return( array);
...
- (PSTCollectionViewCell *)cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    PSTCollectionViewCell *cell = nil;
    id   key;
    ...
    for( key in _visibleViewsDict)
    {
        ...
        if ([itemKey type] == PSTCollectionViewItemTypeCell) {
            if ([[itemKey indexPath] isEqual:indexPath]) {
                cell = [_visibleViewsDict objectForKey:key];
                break;
            }
        }
    }
...
- (void)deleteSections:(NSIndexSet *)sections {
   ...
   range = NSMakeRange( 0, NSIntegerMax);
   while( (length = [sections getIndexes:buffer
                              maxCount:sizeof( buffer) / sizeof( buffer[ 0])
                          inIndexRange:&range]))
   {
      for( idx = 0; idx < length; idx++)
      {
         n = [self numberOfItemsInSection:(NSInteger) buffer[ idx]];
         for ( i = 0; i < n; ++i) 
         {
            path = [NSIndexPath indexPathForItem:i inSection:(NSInteger)buffer[ idx]];
...
- (void)setBackgroundView:(UIView *)backgroundView {
    if (backgroundView != _backgroundView) {
        ...
        [backgroundView setFrame:(CGRect){.origin=[self contentOffset], .size=[self bounds].size}];
...
- (void) _applyNewLayoutBlockWithData:(NSDictionary *) layoutInterchangeData
                               layout:(id) layout
{
   for (PSTCollectionViewItemKey *key in layoutInterchangeData) 
   {
       // TODO: This is most likely not 100% the same time as in UICollectionView. Needs to be investigated.
       PSTCollectionViewCell *cell = (PSTCollectionViewCell *)[_visibleViewsDict objectForKey:key];
       [cell willTransitionFromLayout:_layout toLayout:layout];
       [cell applyLayoutAttributes:[[layoutInterchangeData objectForKey:key] objectForKey:@"newLayoutInfos"]];
       [cell didTransitionFromLayout:_layout toLayout:layout];
   }
}

- (void) _freeUnusedViewsWithNewlyVisibleItemKeys:(NSSet *) newlyVisibleItemsKeys
{
   NSMutableSet * toRemove = [NSMutableSet set];
   for (PSTCollectionViewItemKey *key in _visibleViewsDict) {
       if (![newlyVisibleItemsKeys containsObject:key]) {
           if ([key type] == PSTCollectionViewItemTypeCell) {
               [self reuseCell:[_visibleViewsDict objectForKey:key]];
               [toRemove addObject:key];
           }
           else if ([key type] == PSTCollectionViewItemTypeSupplementaryView) {
               [self reuseSupplementaryView:[_visibleViewsDict objectForKey:key]];
               [toRemove addObject:key];
           }
           else if ([key type] == PSTCollectionViewItemTypeDecorationView) {
               [self reuseDecorationView:[_visibleViewsDict objectForKey:key]];
               [toRemove addObject:key];
           }
       }
   }
   ...
   for (id key in toRemove)
       [_visibleViewsDict removeObjectForKey:key];
}


- (void) animationWhateverDidFinish:(NSDictionary *) context
{
   id        newlyVisibleItemsKeys;
   CGPoint   targetOffset;
   CGRect    contentRect;

   newlyVisibleItemsKeys = [context objectForKey:@"newlyVisibleItemsKeys"];
   targetOffset          = [[context objectForKey:@"targetOffset"] CGPointValue];
   contentRect           = [[context objectForKey:@"contentRect"] CGRectValue];

   // 
   [self _freeUnusedViewsWithNewlyVisibleItemKeys:newlyVisibleItemsKeys];
...
- (void)setCollectionViewLayout:(PSTCollectionViewLayout *)layout animated:(BOOL)animated {
    ...
    if (CGRectIsEmpty([self bounds]) || !_collectionViewFlags.doneFirstLayout) {
        ...
        NSArray * previouslyVisibleItemsKeys = [_visibleViewsDict allKeys];
        ...
        [self bringSubviewToFront:[_visibleViewsDict objectForKey:[previouslyVisibleItemsKeysSetMutable anyObject]]];
        ...
        for (PSTCollectionViewLayoutAttributes *attr in newlyVisibleLayoutAttrs) {
            ...
            if (prevAttr != nil && newAttr != nil) {
                [layoutInterchangeData setObject: @{@"previousLayoutInfos": prevAttr, @"newLayoutInfos": newAttr}
                forKey:newKey];
            }
        }
        ...
        for (PSTCollectionViewItemKey *key in previouslyVisibleItemsKeysSet) {
            ...
            if ([key type] == PSTCollectionViewItemTypeDecorationView) {
                PSTCollectionReusableView *decorView = [_visibleViewsDict objectForKey:key];
                ...
                PSTCollectionReusableView *suuplView = [_visibleViewsDict objectForKey:key];
            ...
            if (prevAttr) [layoutInterchangeDataValue setObject:prevAttr
            forKey:@"previousLayoutInfos"];
            if (newAttr) [layoutInterchangeDataValue setObject:newAttr
            forKey:@"newLayoutInfos"];
            [layoutInterchangeData setObject:layoutInterchangeDataValue forKey:key];
        }

        for (PSTCollectionViewItemKey *key in layoutInterchangeData) {
            if ([key type] == PSTCollectionViewItemTypeCell) {
                PSTCollectionViewCell *cell = [_visibleViewsDict objectForKey:key];
                ...
                if (!cell) {
                    ...
                    [_visibleViewsDict setObject:cell forKey:key];
                ...
                PSTCollectionReusableView *view = [_visibleViewsDict objectForKey:key];
                if (!view) {
                    PSTCollectionViewLayoutAttributes *attrs = [[layoutInterchangeData objectForKey:key] objectForKey:@"previousLayoutInfos"];
                    ...
                    [_visibleViewsDict setObject:view forKey:key];
        ...
        if (animated) 
        {
           ...
           context = @{ 
                        @"newlyVisibleItemsKeys" : newlyVisibleItemsKeys,
                        @"contentRect"  : [NSValue valueWithCGRect:contentRect],
                        @"targetOffset" : [NSValue valueWithCGPoint:targetOffset]
                      };
           [context retain];    
           [UIView beginAnimations:"whatever"
                            context:context];
           {
              [UIView setAnimationDuration:0.3];
              [UIView setAnimationDelegate:self];
              [UIView setAnimationDidStopSelector:@selector( animationWhateverDidFinish:)];

              _collectionViewFlags.updatingLayout = YES;
              [self setContentOffset:targetOffset];
              [self setContentSize:contentRect.size];
              [self _applyNewLayoutBlockWithData:layoutInterchangeData
                                          layout:layout];
           }
...
- (void)setDelegate:(id<PSTCollectionViewDelegate>)delegate {
    [[self extVars] setCollectionViewDelegate:delegate];
...
- (void)updateVisibleCellsNow:(BOOL)now {
    ...
    for (PSTCollectionViewLayoutAttributes *layoutAttributes in layoutAttributesArray) {
        ...
        PSTCollectionReusableView *view = [_visibleViewsDict objectForKey:itemKey];
        if (!view) {
            if ([itemKey type] == PSTCollectionViewItemTypeCell) {
            ...
            if (view) {
                [_visibleViewsDict setObject:view forKey:itemKey];
                [self addControlledSubview:view];
    ...
    NSMutableSet * allVisibleItemKeys = [NSMutableSet setWithArray:[_visibleViewsDict allKeys]];
    ...
    for (PSTCollectionViewItemKey *itemKey in allVisibleItemKeys) {
        PSTCollectionReusableView *reusableView = [_visibleViewsDict objectForKey:itemKey];
        if (reusableView) {
            [reusableView removeFromSuperview];
            [_visibleViewsDict removeObjectForKey:itemKey];
            if ([itemKey type] == PSTCollectionViewItemTypeCell) {
...
- (void)queueReusableView:(PSTCollectionReusableView *)reusableView inQueue:(NSMutableDictionary * )queue withIdentifier:(NSString * )identifier {
    ...
    // enqueue cell
    NSMutableArray * reuseableViews = [queue objectForKey:identifier];
    if (!reuseableViews) {
...
- (void)addControlledSubview:(PSTCollectionReusableView *)subview {
    ...
    NSMutableArray * floatingViews = [NSMutableArray array];
    ...
    [floatingViews sortUsingSelector:@selector( compare:)];
...
- (void)updateWithItems:(NSArray * )items {
    ...
    NSMutableDictionary * newAllVisibleView = [NSMutableDictionary dictionary];
    ...
    for (PSTCollectionViewUpdateItem *updateItem in items) {
        ...
        if ([updateItem isSectionOperation] && [updateItem updateAction] == PSTCollectionUpdateActionDelete) {
            NSInteger numberOfBeforeSection = [[_currentUpdate objectForKey:@"oldModel"] numberOfItemsInSection:[[updateItem indexPathBeforeUpdate] section]];
            for (NSInteger i = 0; i < numberOfBeforeSection; i++) {
                ...
                if (view) {
                    ...
                    if (!finalAttrs) {
                        finalAttrs = [[startAttrs copy] autorelease];
                    ...
                    [(NSMutableArray* ) [viewsToRemove objectForKey:@([key type])] addObject:view];
        ...
        if ([updateItem updateAction] == PSTCollectionUpdateActionDelete) {
            ...
            if (CGRectIntersectsRect([self visibleBoundRects], startRect) || CGRectIntersectsRect([self visibleBoundRects], finalRect)) {
                ...
                if (!startAttrs) {
                    startAttrs = [[finalAttrs copy] autorelease];
            ...
            PSTCollectionReusableView *view = [_visibleViewsDict objectForKey:keyBefore];
            ...
            if (view) {
                ...
                [_visibleViewsDict removeObjectForKey:keyBefore];
    ...
    for (PSTCollectionViewItemKey *key in _visibleViewsDict) {
        ...
        if ([key type] == PSTCollectionViewItemTypeCell) {
            NSUInteger oldGlobalIndex = [[_currentUpdate objectForKey:@"oldModel"] globalIndexForItemAtIndexPath:[key indexPath]];
            NSArray * oldToNewIndexMap = [_currentUpdate objectForKey:@"oldToNewIndexMap"];
            NSUInteger newGlobalIndex = NSNotFound;
            if (NSNotFound != oldGlobalIndex && oldGlobalIndex < [oldToNewIndexMap count]) {
                newGlobalIndex = [[oldToNewIndexMap objectAtIndex:oldGlobalIndex] unsignedIntegerValue];
            }
            NSIndexPath *newIndexPath = newGlobalIndex == NSNotFound ? nil : [[_currentUpdate objectForKey:@"newModel"] indexPathForItemAtGlobalIndex:(int)newGlobalIndex];
            NSIndexPath *oldIndexPath = oldGlobalIndex == NSNotFound ? nil : [[_currentUpdate objectForKey:@"oldModel"] indexPathForItemAtGlobalIndex:(int)oldGlobalIndex];
            ...
            [animations addObject:dic];
            PSTCollectionViewItemKey *newKey = [[key copy] autorelease];
            [newAllVisibleView setObject:view forKey:newKey];
        }
    }
    ...
    for (PSTCollectionViewLayoutAttributes *attrs in allNewlyVisibleItems) {
        ...
        if ([key type] == PSTCollectionViewItemTypeCell && ![[newAllVisibleView allKeys] containsObject:key]) {
            ...
            PSTCollectionReusableView *view = [self createPreparedCellForItemAtIndexPath:[attrs indexPath]
                    withLayoutAttributes:startAttrs];
            [self addControlledSubview:view];
            [newAllVisibleView setObject:view forKey:key]; // (nat) TODO: check!
    ...
    _visibleViewsDict = newAllVisibleView;

    for (NSDictionary * animation in animations) {
        PSTCollectionReusableView *view = [animation objectForKey:@"view"];
        PSTCollectionViewLayoutAttributes *attr = [animation objectForKey:@"previousLayoutInfos"];
...
static void    update_operations_count( NSMutableDictionary *operations, 
                                        id sectionKey, 
                                        NSString *countKey)
{
   ...
   dictSection = [operations objectForKey:sectionKey];
   if (!dictSection)
   {
      dictSection = [NSMutableDictionary dictionary];
      [operations setObject:dictSection
                     forKey:sectionKey];
   }
   n = [[dictSection objectForKey:countKey] intValue] + 1;
   [dictSection setObject:@(n)
                   forKey:countKey];   
}
...
- (void)endItemAnimations {
    ...
    _collectionViewData = [[[PSTCollectionViewData alloc] initWithCollectionView:self layout:_layout] autorelease];

    [_layout invalidateLayout];
    [_collectionViewData prepareToLoadData];

    NSMutableArray * someMutableArr1 = [NSMutableArray array];
    ...
    NSMutableArray * sortedMutableReloadItems = [[[_reloadItems sortedArrayUsingSelector:@selector(compareIndexPaths:)] mutableCopy] autorelease];
    NSMutableArray * sortedMutableMoveItems = [[[_moveItems sortedArrayUsingSelector:@selector(compareIndexPaths:)] mutableCopy] autorelease];

    _originalDeleteItems = [[removeUpdateItems copy] autorelease];
    _originalInsertItems = [[insertUpdateItems copy] autorelease];

    NSMutableArray * someMutableArr2 = [NSMutableArray array];
    NSMutableArray * someMutableArr3 = [NSMutableArray array];
    ...
    for (PSTCollectionViewUpdateItem *updateItem in sortedMutableReloadItems) {
        NSAssert2([[updateItem indexPathBeforeUpdate] section] < [oldCollectionViewData numberOfSections],
        ...
        if ([[updateItem indexPathBeforeUpdate] item] != NSNotFound) {
            NSAssert3([[updateItem indexPathBeforeUpdate] item] < [oldCollectionViewData numberOfItemsInSection:[[updateItem indexPathBeforeUpdate] section]],
    ...
    NSMutableArray * sortedDeletedMutableItems = [[[_deleteItems sortedArrayUsingSelector:@selector(inverseCompareIndexPaths:)] mutableCopy] autorelease];
    NSMutableArray * sortedInsertMutableItems = [[[_insertItems sortedArrayUsingSelector:@selector(compareIndexPaths:)] mutableCopy] autorelease];

    for (PSTCollectionViewUpdateItem *deleteItem in sortedDeletedMutableItems) {
        if ([deleteItem isSectionOperation]) {
            NSAssert2([[deleteItem indexPathBeforeUpdate] section] < [oldCollectionViewData numberOfSections],
            ...
            for (PSTCollectionViewUpdateItem *moveItem in sortedMutableMoveItems) {
                if ([[moveItem indexPathBeforeUpdate] section] == [[deleteItem indexPathBeforeUpdate] section]) {
                    ...
                    NSAssert1(NO, @"attempt to delete and move from the same section %ld", (long)[[deleteItem indexPathBeforeUpdate] section]);
                    else
                    NSAssert1(NO, @"attempt to delete and move from the same section (%@)", [moveItem indexPathBeforeUpdate]);
                }
            }
        }else {
            NSAssert2([[deleteItem indexPathBeforeUpdate] section] < [oldCollectionViewData numberOfSections],
            ...
            (long)[oldCollectionViewData numberOfSections]);
            NSAssert3([[deleteItem indexPathBeforeUpdate] item] < [oldCollectionViewData numberOfItemsInSection:[[deleteItem indexPathBeforeUpdate] section]],
            ...
            for (PSTCollectionViewUpdateItem *moveItem in sortedMutableMoveItems) {
                NSAssert1(![[deleteItem indexPathBeforeUpdate] isEqual:[moveItem indexPathBeforeUpdate]],
                @"attempt to delete and move the same item (%@)", [deleteItem indexPathBeforeUpdate]);
            }

            update_operations_count( operations, @([[deleteItem indexPathBeforeUpdate] section]), @"deleted");
        }
    }
    ...
    for (NSUInteger i = 0; i < [sortedInsertMutableItems count]; i++) {
        ...
        if (sectionOperation) {
            NSAssert2([indexPath section] < [_collectionViewData numberOfSections],
            ...
            for (PSTCollectionViewUpdateItem *moveItem in sortedMutableMoveItems) {
                if ([[moveItem indexPathAfterUpdate] isEqual:indexPath]) {
                    if ([moveItem isSectionOperation])
                    NSAssert1(NO, @"attempt to perform an insert and a move to the same section (%ld)", (long)[indexPath section]);
                }
            }

            NSUInteger j = i + 1;
            while (j < [sortedInsertMutableItems count]) {
                PSTCollectionViewUpdateItem *nextInsertItem = [sortedInsertMutableItems objectAtIndex:j];

                if ([[nextInsertItem indexPathAfterUpdate] section] == [indexPath section]) {
                    NSAssert4([[nextInsertItem indexPathAfterUpdate] item] < [_collectionViewData numberOfItemsInSection:[indexPath section]],
            ...
            NSAssert3([indexPath item] < [_collectionViewData numberOfItemsInSection:[indexPath section]],
            @"attempt to insert item to (%@) but there are only %ld items in section %ld after update",
            indexPath,
            (long)[_collectionViewData numberOfItemsInSection:[indexPath section]],
            (long)[indexPath section]);

            update_operations_count( operations, @([indexPath section]), @"inserted");
        }
    }

    for (PSTCollectionViewUpdateItem *sortedItem in sortedMutableMoveItems) {
        if ([sortedItem isSectionOperation]) {
            NSAssert2([[sortedItem indexPathBeforeUpdate] section] < [oldCollectionViewData numberOfSections],
            @"attempt to move section (%ld) that doesn't exist (%ld sections before update)",
            (long)[[sortedItem indexPathBeforeUpdate] section],
            (long)[oldCollectionViewData numberOfSections]);
            NSAssert2([[sortedItem indexPathAfterUpdate] section] < [_collectionViewData numberOfSections],
            ...
            NSAssert3([[sortedItem indexPathBeforeUpdate] item] < [oldCollectionViewData numberOfItemsInSection:[[sortedItem indexPathBeforeUpdate] section]],
            ...
            (long)[_collectionViewData numberOfSections]);
            NSAssert3([[sortedItem indexPathAfterUpdate] item] < [_collectionViewData numberOfItemsInSection:[[sortedItem indexPathAfterUpdate] section]],
        ...
        update_operations_count( operations, @([[sortedItem indexPathBeforeUpdate] section]), @"movedOut");
        update_operations_count( operations, @([[sortedItem indexPathAfterUpdate] section]), @"movedIn");
    }

#if !defined  NS_BLOCK_ASSERTIONS
    for (NSNumber *sectionKey in operations) {
        NSInteger section = [sectionKey integerValue];

        NSInteger insertedCount = [[[operations objectForKey:sectionKey] objectForKey:@"inserted"] integerValue];
        NSInteger deletedCount = [[[operations objectForKey:sectionKey] objectForKey:@"deleted"] integerValue];
        NSInteger movedInCount = [[[operations objectForKey:sectionKey] objectForKey:@"movedIn"] integerValue];
        NSInteger movedOutCount = [[[operations objectForKey:sectionKey] objectForKey:@"movedOut"] integerValue];
    ...
    NSMutableArray * newModel = [NSMutableArray array];
    {
       ...
       n = [oldCollectionViewData numberOfSections];
       for (NSInteger i = 0; i < n; i++) {
           NSMutableArray * sectionArr = [NSMutableArray array];
           m = [oldCollectionViewData numberOfItemsInSection:i];
           for (NSInteger j = 0; j < m; j++)
           {
               indexPath   = [NSIndexPath indexPathForItem:j inSection:i]; 
               globalIndex = [oldCollectionViewData globalIndexForItemAtIndexPath:indexPath]; 
               [sectionArr addObject:@(globalIndex)];
           }
        ...
        switch ([updateItem updateAction]) {
            case PSTCollectionUpdateActionDelete: {
                if ([updateItem isSectionOperation]) {
                    ...
                    [(NSMutableArray *) [newModel objectAtIndex:(NSUInteger)[[updateItem indexPathBeforeUpdate] section]]
            ...
            case PSTCollectionUpdateActionInsert: {
                if ([updateItem isSectionOperation]) {
                    [newModel insertObject:[NSMutableArray array]
                                   atIndex:(NSUInteger)[[updateItem indexPathAfterUpdate] section]];
                }else {
                    [(NSMutableArray *) [newModel objectAtIndex:(NSUInteger)[[updateItem indexPathAfterUpdate] section]]
            ...
            case PSTCollectionUpdateActionMove: {
                if ([updateItem isSectionOperation]) {
                    id section = [newModel objectAtIndex:(NSUInteger)[[updateItem indexPathBeforeUpdate] section]];
                    ...
                    [[newModel objectAtIndex:(NSUInteger)[[updateItem indexPathBeforeUpdate] section]] removeObject:object];
                    [[newModel objectAtIndex:(NSUInteger)[[updateItem indexPathAfterUpdate] section]] insertObject:object
                                                                            atIndex:(NSUInteger)[[updateItem indexPathAfterUpdate] item]];
                }
            }
                break;
...

    {
       ...
       n = [oldCollectionViewData numberOfItems];
       for (NSInteger i = 0; i < n; i++)
           [oldToNewMap addObject:@(NSNotFound)];

       n = [_collectionViewData numberOfItems];
       for (NSInteger i = 0; i < n; i++)
       ...
       for (NSUInteger i = 0; i < n; i++) {
           ...
           for (NSUInteger j = 0; j < m; j++) {
               indexPath = [NSIndexPath indexPathForItem:(NSInteger)j inSection:(NSInteger)i];
               newGlobalIndex = [_collectionViewData globalIndexForItemAtIndexPath:indexPath];
               if ([[section objectAtIndex:j] integerValue] != NSNotFound)
                    [oldToNewMap replaceObjectAtIndex:[[section objectAtIndex:j] unsignedIntegerValue]
                    withObject:@(newGlobalIndex)];
               if (newGlobalIndex != NSNotFound)
                    [newToOldMap replaceObjectAtIndex:newGlobalIndex
                    withObject:[section objectAtIndex:j]];
           }
       }
    }

    _currentUpdate = @{@"oldModel" : oldCollectionViewData, @"newModel" : _collectionViewData, @"oldToNewIndexMap" : oldToNewMap, @"newToOldIndexMap" : newToOldMap};
...
- (void)updateRowsAtIndexPaths:(NSArray * )indexPaths updateAction:(PSTCollectionUpdateAction)updateAction {
    ...
    for (NSIndexPath *indexPath in indexPaths) {
        PSTCollectionViewUpdateItem *updateItem = [[[PSTCollectionViewUpdateItem alloc] initWithAction:updateAction forIndexPath:indexPath] autorelease];
...
- (void)updateSections:(NSIndexSet *)sections updateAction:(PSTCollectionUpdateAction)updateAction {
   ...
   updating = _collectionViewFlags.updating;
   if (!updating) [self setupCellAnimations];

   NSMutableArray * updateActions;
    
   updateActions = [self arrayForUpdateAction:updateAction];
                          ...
                          inIndexRange:&range]))
   {
      for( idx = 0; idx < length; idx++)
      {
        path = [NSIndexPath indexPathForItem:NSNotFound inSection:buffer[ idx]];
        updateItem = [[PSTCollectionViewUpdateItem alloc] initWithAction:updateAction 
                                                            forIndexPath:path];
